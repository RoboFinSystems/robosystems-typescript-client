...
- **Corroborate against the branch:**
  ...
  ```
- **Hard rule — no confabulation.** Every claim in the description must be supported by the diff. If you didn't change the public API surface, don't write "new client methods." If a behavior isn't in the diff, don't mention it. When the session context and the diff disagree, the diff wins and you investigate the discrepancy. This is a published TypeScript SDK — be especially precise about what changed in the generated clients, types, and hand-written wrappers, since consumers depend on it.
...
- **Title** — concise (~50–72 chars), conventional-commit style with a scope, matching the style in `git log` (e.g. `feat(ledger): add report bundle download support`, `chore(sdk): regenerate against report-bundle endpoint`).
- **Body** — markdown, only sections that apply:
  - **Summary** — 1–3 sentences: what this PR does and why.
  - **Changes** — bullets grouped by area/file, describing real edits (e.g. regenerated SDK vs. hand-written client wrappers vs. tooling).
  - **Testing** — state truthfully what was run. If `npm run test:all` (or a subset like `npm run validate`, `npm run test`, `npm run build`) was run this session, say so and give the result. If nothing was run, say "Not run" — never claim passing tests that weren't executed.
  - **Notes / Follow-ups** — optional: deferred items, breaking changes for consumers, related issues/specs. Note that version bumps and publishing are owned by the user — do not imply this PR publishes a release.
