 ...
 * Delete Connection
 *
 * Removes the connection and revokes credentials. Imported data is preserved in the graph. Requires admin role. `disposition=sever` (QuickBooks only) is the cutover to native books: the chart QuickBooks created becomes the tenant's own and QuickBooks can never resume over it; the default `disconnect` keeps the connection reconnectable.
 */
 ...
 *
 * Link a chart-of-accounts element to a US GAAP reporting concept. One mapping edge per call — use `auto-map-elements` for bulk AI-assisted mapping. Duplicate (from, to, type) tuples return 409. Map before you close: an account with landed history in a closed month cannot be re-mapped (422) — the closed month's stamped statements were computed through the old arcs. Reopen latest-first down to the earliest month named, map, then close forward.
 *
 * **Idempotency**: supply an `Idempotency-Key` header to make safe retries; replays within 24 hours return the same envelope. Reusing the key with a different body returns HTTP 409 Conflict.
 */
 ...
 *
 * Remove a single CoA → reporting-concept mapping edge by id. The mapping structure itself remains; only the association row is dropped. Use this to correct a wrong mapping — delete the bad edge, then `create-mapping-association` the right one. Find the association id via the `library_element_arcs` GraphQL field. Library-seeded rows cannot be deleted (403). An edge for an account with landed history in a closed month cannot be removed (422) until those months are reopened, latest-first.
 *
 * **Idempotency**: supply an `Idempotency-Key` header to make safe retries; replays within 24 hours return the same envelope. Reusing the key with a different body returns HTTP 409 Conflict.
 */
 ...
 * Disclosures
 *
 * The map of a report's sections: one row per disclosure family — a note with its policies, tables and details, a statement with its parenthetical, the cover page — with block counts by level, fact and text-block counts, in filing order; with `topic`, one family's blocks with the ids `information-block` takes. Reads the report whole — the published filing on the SEC shared repository, the ledger's own report on a tenant graph — into xbrlkit's model and runs xbrlkit's own `disclosures`, so both answer as `xbrlkit serve` does. Cheap: call it before `information-block`. Shared-repo graphs take `ticker` (auto-resolving the latest filing) or `report_id`; tenant graphs take `report_id`. A filing processed before its artifacts existed answers 404 until the repository is reprocessed.
 *
 * **Idempotency**: supply an `Idempotency-Key` header to make safe retries; replays within 24 hours return the same envelope. Reusing the key with a different body returns HTTP 409 Conflict.
 */
export const disclosures = <ThrowOnError extends boolean = false>(options: Options<DisclosuresData, ThrowOnError>): RequestResult<DisclosuresResponses, DisclosuresErrors, ThrowOnError> => (options.client ?? client).post<DisclosuresResponses, DisclosuresErrors, ThrowOnError>({
    security: [{ name: 'X-API-Key', type: 'apiKey' }, { scheme: 'bearer', type: 'http' }],
 ...
 * Information Block
 *
 * One section of a report read whole — the expensive call: rows in presentation order with the consolidated value per period column, the same rows broken out by the section's own axes, the axes with the members that carry facts, every total's calculation children with a footing check, and the section's text blocks. Member breakdowns and period columns are kept most-reported / most-recent first up to a response budget; a row is never left blank by a cut, and `members_omitted` / `periods_omitted` say what was. A block longer than `max_rows` is `truncated`; pass its `next_offset` as `offset` for the next page. Take `block` from `disclosures`. Same resolution as `disclosures`: `ticker` or `report_id` on shared-repo graphs, `report_id` on tenant graphs. On a tenant graph this reads the section as the ledger's report holds it; `get-information-block` returns one authored block's envelope with its rules and verification.
 *
 * **Idempotency**: supply an `Idempotency-Key` header to make safe retries; replays within 24 hours return the same envelope. Reusing the key with a different body returns HTTP 409 Conflict.
 */
