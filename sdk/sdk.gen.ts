 ...
 * Delete Graph
 *
 * Permanently destroys a user graph: cancels its subscription immediately and queues fast-path deprovisioning (LadybugDB database removed, DynamoDB slot freed, PG records cleaned). Requires `confirm` to equal the URL `graph_id`. Caller must be admin on the graph. Not allowed on shared repositories. Deprovisioning typically completes within ~10 minutes; poll the graph status to verify.
 *
 * **Idempotency**: supply an `Idempotency-Key` header to make safe retries; replays within 24 hours return the same envelope. Reusing the key with a different body returns HTTP 409 Conflict.
 */
